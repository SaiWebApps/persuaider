...
const ACCEPTING = /\b(offer|offering|approve|approved|authorize|authorized|agree|agreed|accept|counter(?:offer)?|propose|propos(?:e|ing)|can do|could do|will do|would do|give you|go to|go up to|come up to|stretch to|meet you at|bring you to|make it|raise you to|put you at|increase (?:it )?to|move (?:you )?to|get you to|confirm|sign off|settle (?:on|at))\b/i;
const NEGATED = /\b(not|n't|never|no|nothing|without|refuse|decline|cannot|can['\u2019]t|won['\u2019]t|don['\u2019]t|doesn['\u2019]t|isn['\u2019]t|haven['\u2019]t|wouldn['\u2019]t|couldn['\u2019]t)\b/i;

/** "$118,000", "$118000", "$118k", "118,000 dollars" → 118000 */
function dollarFigures(text: string): number[] {
  const out: number[] = [];
  for (const m of text.matchAll(/\$\s?(\d{1,3}(?:,\d{3})+|\d{4,7})(?!\d)|\$\s?(\d{2,3})\s?k\b|\b(\d{1,3}(?:,\d{3})+|\d{4,7})\s+dollars\b/gi)) {
